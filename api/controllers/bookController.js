...
export const books = async (_, reply) => {
	const books = await db.select().from(booksTable).orderBy(desc(booksTable.id));
...
export const storeBook = async (request, reply) => {
	try {
			...
			return reply
				...
				.send(
					ApiResponse(
						402,
						"Validation error",
						z.flattenError(form.error)?.fieldErrors,
					),
				);
		}
		...
		reply.code(500).send(ApiResponse(500, error.message));
	}
};
